		...
		>
			<nav className="mx-auto flex h-16 w-full max-w-5xl items-center justify-between px-4">
				...
				<div className="hidden items-center gap-4 md:flex">
					{links.map((link) => (
						<a key={link.label} className={cn(buttonVariants({ variant: 'ghost' }), "text-sm font-medium text-muted-foreground hover:text-primary")} href={link.href}>
							{link.label}
						</a>
					))}
	...
	return createPortal(
		<div
			...
			className={cn(
				...
				'fixed top-16 right-0 bottom-0 left-0 z-40 flex flex-col overflow-hidden border-t border-white/5 md:hidden',
			)}
		>
			<div
